...
try:
    # Загружаем карту TMX
    tmx_map = pytmx.load_pygame('безымянный.tmx', pixelalpha=True)
    camera_x = 0
    camera_y = 0

    # Извлекаем слой плитки
    tile_layer = tmx_map.layers[0]
    ...
    # Размер всей карты
    map_width = tmx_map.width * tile_width
    map_height = tmx_map.height * tile_height

    # Рендеринг всех плиток слоя
    for x, y, image in tile_layer.tiles():
        screen.blit(image, (x * tile_width - camera_x, y * tile_height - camera_y))
